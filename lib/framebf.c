 ...
 */
void framebf_init(unsigned int physical_width, unsigned int physical_height,
                  unsigned int virtual_width, unsigned int virtual_height,
...
void drawLineWithThicknessARGB32(int x1, int y1, int x2, int y2,
                                 unsigned int attr, int thickness) {
  ...
  for (float i = -halfThickness; i <= halfThickness; i += 0.5f) {
    ...
    drawLineARGB32(x1 + xOffset, y1 + yOffset, x2 + xOffset, y2 + yOffset,
                   attr);
  }
}

void drawCircleARGB32(int x_center, int y_center, int radius, unsigned int attr,
                      int fill) {
  int x = 0;
  int y = radius;
  int d = 3 - 2 * radius;
...
void drawImage(const unsigned long *img, int x, int y, int img_pixels_width,
               int img_pixels_height) {
...
void drawVideo(int x, int y, const unsigned long **video,
               int video_pixels_width, int video_pixels_height,
               int video_length) {
...
void drawBrickBlock(int x, int y) {
  drawImage(brickBlock, x, y, block_width, block_height);
}
